...
function MenuSlide({
  ...
  const [menudefult, setMenudefult] = useState<MenusInterface>({
    ID: 0,
    MenuID: 0,
    MenuName: "coming soon . . .",
    MenuNameEng: "coming soon . . .",
    MenuCost: 0,
    MenuImage: undefined,
    MenuStatus: 0,
    MenuTypeID: 0,
  ...
  if (menusReady.length >= 3) {
    menushow[0] = menusReady[index1];
    menushow[1] = menusReady[index2];
    menushow[2] = menusReady[index3];
    setMenushow(menushow[1]);
  } else if (menusReady.length == 2) {
    menushow = [menusReady[0], menusReady[0], menusReady[1]];
    setMenushow(menushow[1]);
  } else if (menusReady.length == 1) {
    menushow = [menudefult, menusReady[0], menudefult];
    setMenushow(menushow[1]);
  } else {
    setMenushow(menudefult);
  }
  useEffect(() => {
    if (menusReady.length >= 3) {
      menushow[0] = menusReady[index1];
  ...
  return (
    <>
          ...
          {menushow.length > 0 &&
            menushow.map((menu, index) => (
              <div key={index} className={`menu-imge i${index}`}>
                {menu?.MenuImage && <img src={menu.MenuImage ?? ""} alt="" />}
              </div>
            ))}
        </div>
        <button
      ...
      <div className="menu-name">
        {menushow.length > 0 && (
          <>
            <p>{menushow[1].MenuCost ?? 0} bath.</p>
            <span>
              {menushow[1].MenuName ?? ""}
              <span> </span>
